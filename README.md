...
- `cmd/server/gcal.go` — Google Calendar API client + connect/disconnect + busy times; `gcal_sync.go` — booking → event mapping and the reconcile job; `gcal_admin.go` — `/admin/calendar/settings`
...
- `db/schema.sql` (embedded), `db/queries.sql` → `sqlc generate` → `db/sqlc/`; wrappers in `db/db.go`, `bookings.go`, `customers.go`, `invoices.go`, `gcal.go`
...
| Calendar sync | every tick, if anything changed | Google Calendar | `bookings.gcal_synced_at` |
...
Every booked visit is pushed to a dedicated **Local IT Help** calendar on the
admin's Google account, and everything else on that account comes back as
anonymous busy blocks on `/admin/calendar`. Off until connected, so an install
without it behaves exactly as before.

Set it up from **`/admin/calendar/settings` → Connect Google Calendar**. The
calendar is created on first connect and existing visits are pushed straight
...
2. Add the scope `https://www.googleapis.com/auth/calendar` to the OAuth
   consent screen.
3. Add the redirect URI `https://localithelp.com.au/auth/google/calendar/callback`
   (plus a `http://localhost:PORT/...` one for dev).
4. **Set publishing status to "In production".** A "Testing" app's refresh
   tokens expire after 7 days, which would silently stop the sync every week.
   Publishing without verification is fine for a single owner account — Google
   shows an "unverified app" warning once, at connect time.

How it behaves:

- **Push triggers.** Scheduling, rescheduling, status changes (including
  cancel), notes and address edits push immediately in the background, and the
  invoice actions that move a booking to `invoiced`/`paid`/`done` do too. A
  booking never fails to save because Google is unreachable — the error is
  logged and shown on the settings page.
- **Reconcile.** Every scheduler tick re-pushes anything whose row changed
  since its last successful push, or whose state disagrees with Google (a visit
  with no event, an event on a cancelled visit). That is the safety net for a
  failed immediate push, so nothing needs a retry queue.
- **One-way for event content.** The app is the source of truth: dragging an
  event inside Google Calendar does *not* move the booking. Reschedule from
  `/admin/bookings/{id}` so the customer gets the confirmation email.
- **Cancelling** (or clearing the start time, or marking spam) deletes the
  event; an event already deleted in Google counts as done. A visit whose event
  was deleted by hand in Google is recreated on the next push.
- **Busy times** come from `freebusy.query`, which returns intervals only —
  never titles, so nothing personal reaches the app. The app's own calendar is
  excluded (its events are the bookings already on the page). Results are
  cached for 2 minutes with a 3-second timeout; on failure the week still
  renders, with a one-line banner.
- **Calendar reminders are off** on pushed events — the app already sends its
  own 1-hour heads-up email.

The refresh token is stored in plaintext in `google_calendar.refresh_token`, so
it is inside the nightly S3 backup. That is an accepted trade-off for a
single-user app; **Disconnect** deletes the row (and forgets every event id,
leaving the events in Google), and access can also be revoked at
